...
class Case(tb.IsDescription):
    ...
    dur_E = tb.UInt32Col()
    dur_I = tb.UInt32Col()
    initial_dur_I = tb.UInt32Col() #nefel
    ID = tb.UInt32Col()
...
class CaseObserver(Observer):
    ...
    """
    def __init__(self, h5file, t_per_year=364, entry_state='I', store_recovery=False, store_source=True, 
                 vaccination_times = [] ):
        self.age_dists = None
        self.age_dist = None
        ...
        self.vaccination_times = vaccination_times #nefel
        if self.store_recovery:
            if not len(self.vaccination_times):
                print("vaccination_times list is empty")
                exit()
            
            self.open_cases = defaultdict(list)  # open cases
            self.open_cases_list = []
    ...
    def _add_new_case(self, t, disease, pop, ind):
        ...
        if self.store_recovery:
            ...
            self.row['initial_dur_I'] = ind.durations.get('I', 0)
        self.row['ID'] = ind.ID
    ...
    def update(self, t, disease, pop, cases, introductions=None, **kwargs):
        ...
        for ind in (cases + introductions) if introductions else cases:
            self._add_new_case(t, disease, pop, ind)
            if self.store_recovery and t <= self.vaccination_times[-1]:
                # build a list of individuals who will need their infectious durations updated when available
                #self.open_cases[t].append(ind.ID)
                self.open_cases_list.append(ind.ID)
...
#            self.age_dist_month['dist'] = pop.age_dist_month(bin_days=91, max_age=5, norm=False)[0]
        ...
        if self.store_recovery and t in self.vaccination_times:
            # 're'-process individuals moving from 'E' to 'I' to add in their infectious duration
            # ('t_R') time entering R
            #print('vaccination time')
            removed_indexes = []
            for index, cur_id in enumerate(self.open_cases_list): #nefel
                cur_ind = pop.I.get(cur_id, None)
                #nefel
                if cur_ind and t in cur_ind.vaccine_times and cur_ind.vaccines_effective[cur_ind.vaccine_times.index(t)]:
                    source_row = self.data.get_where_list('ID == %s' % cur_id)[-1]
                    if self.data.cols.t_R[source_row] > t:
                        #print("%s,  %s "%(t, cur_id)) 
                        self.data.cols.t_R[source_row] = t 
                        self.data.cols.dur_I[source_row] = t - self.data.cols.t_E[source_row] 
                    
                    removed_indexes.append(index)
                     
                        
                if not cur_ind:
                    #person died and do not change its infection time only remove person from open list
                    ...
                    removed_indexes.append(index)
            self.open_cases_list = [i for j, i in enumerate(self.open_cases_list) if j not in removed_indexes]
                #for ind in removed_ind:
                #    self.open_cases_list.remove(ind)
                

            # remove current time step's open cases
            #del self.open_cases[t] #nefel
