...
class SummaryStatsObserver(Observer):
    """
    An observer for recording summary stats. Imputed population is used for summary statistics.
    """
    def __init__(self,h5file,observefrom,et,fname,rng, t_per_year=364):
        self.rng=rng
        self.fname=fname
    ...
    def update(self, t, disease, pop,cases, **kwargs):
        ...
        if t % self.t_per_year in range(0, self.t_per_year, self.t_per_year // 4):
            if t >= (self.observefrom):
                ...
                all_hh_ids=list(set([i.groups['household'] for i in list(pop.I.values())]))
                while True:
                    sample_hh_ids=self.rng.sample(all_hh_ids,530)
                    all_peoplein_sample_hhs=[i for i in list(pop.I.values()) if i.groups['household'] in sample_hh_ids]
                    sample_size=1300
                    sampled_pop=self.rng.sample(all_peoplein_sample_hhs,min(sample_size,len(all_peoplein_sample_hhs)))
                    no_hhs_sampled_pop=len(list(set([i.groups['household'] for i in sampled_pop])))
                    #display(no_hhs_sampled_pop)
                    if no_hhs_sampled_pop in range(450, 500):
                        break
                #display(no_hhs_sampled_pop)
                
                self.age_dist_prevalence['t'] = t
                ages_infected= np.array([i.age for i in sampled_pop if i.state.label == "I"])
                ages_infected_hist=np.histogram(ages_infected, bins=16, range=(0, 80))[0]
                ages_all= np.array([i.age for i in sampled_pop])
                ...
                hh_size_infected=np.array([hh_size(pop, i) for i in sampled_pop if i.state.label == "I"])
                hh_size_infected_hist=np.histogram(hh_size_infected, bins=12, range=(1,13))[0]
                hh_size_all= np.array([hh_size(pop, i) for i in sampled_pop])
                ...
                #other simple stats
                self.stat['t']=t
                self.stat['overallprevalence']=len([i for i in sampled_pop if i.state.label == "I"])/len(list(pop.I.values()))
                ages_infected=np.array([i.age for i in sampled_pop if i.state.label == "I"])
                ...
                hhsizes_infected=np.array([hh_size(pop, i) for i in sampled_pop if i.state.label == "I"])
                ...
                if t/self.t_per_year == round(self.et/self.t_per_year): #final saving moment #save documents into csv files
                    ...
                    self.age_comm_dists_prevalence_df.to_csv('%s_agecommdists.csv'%(self.fname[:-4]), index=False)
                    self.hh_size_comm_dists_prevalence_df.to_csv('%s_hhcommdists.csv'%(self.fname[:-4]), index=False)
...
class SummaryStatsObserver2(Observer):
    """
    An observer for recording summary stats. new summary statistics hh with at least 1,2,3 cases are recorded.
    It collects data for the imputed datasets where all the household members are recorded for the sampled households.
    """
    def __init__(self,h5file,observefrom,et,fname,rng,  t_per_year=364):
        self.rng=rng
        self.fname=fname
        ...
        super(SummaryStatsObserver2, self).__init__(h5file, 'stats', desc, 'Summary Stats') #??? 
    ...
    def create_storage(self, description, title): #Q.Shapes?
        ...
        self.stats = self.h5file.create_table(group, 'stats',
            ...
            'mean_age_infected': tb.Float32Col(shape=(1)),
             ...
             'mean_hhsize_infected':tb.Float32Col(shape=(1)),
             'hh_with_zerocase':tb.Float32Col(shape=(1)),
             'hh_with_atleastonecase':tb.Float32Col(shape=(1)),
             'hh_with_atleasttwocase':tb.Float32Col(shape=(1)),
             'hh_with_atleastthreecase':tb.Float32Col(shape=(1)),
             'com_transmission_percentage':tb.Float32Col(shape=(1)),
            }, 'single Statistics')
        self.stat=self.stats.row
    ...
    def update(self, t, disease, pop,cases, **kwargs):
        ...
        if t % self.t_per_year in range(0, self.t_per_year, self.t_per_year // 4):
            if t >= (self.observefrom):
                ...
                #no_hhs_sampled_pop=len(list(set([i.groups['household'] for i in sampled_pop])))
                sampled_hh=481
                sample_hh_ids=self.rng.sample(all_hh_ids,sampled_hh)
                all_peoplein_sample_hhs=[i for i in list(pop.I.values()) if i.groups['household'] in sample_hh_ids]
                sampled_pop=all_peoplein_sample_hhs
                no_hhs_sampled_pop=len(list(set([i.groups['household'] for i in sampled_pop])))
                #while True:
                #    sample_hh_ids=self.rng.sample(all_hh_ids,481)
                #    all_peoplein_sample_hhs=[i for i in list(pop.I.values()) if i.groups['household'] in sample_hh_ids]
                    #sample_size=1300
                    #sampled_pop=self.rng.sample(all_peoplein_sample_hhs,min(sample_size,len(all_peoplein_sample_hhs)))
                    #no_hhs_sampled_pop=len(list(set([i.groups['household'] for i in sampled_pop])))
                ...
                #hh with 0,1,2 cases
                number_hhs=no_hhs_sampled_pop#len(pop.households) #number of total hhs in pop
                #display(list(pop.groups['household'].keys())) #list of hh ids
                #display(len(list(pop.groups['household'].keys()))) #len of hhs
                hhs_with_zerocases=0
                hhs_with_atleastonecase=0
                hhs_with_atleasttwocases=0
                hhs_with_atleastthreecases=0
                
                for x in sample_hh_ids:
                    #display(x)
                    cur_hh_members=[i for i in list(pop.I.values()) if i.groups['household'] == x]
                    #cur_hh_members= sampled_pop.groups['household'][x]
                    #display(cur_hh_members)
                    num_cases=[1 if x.state.label=="I" else 0 for x in cur_hh_members]
                    #display(num_cases)
                    if sum(num_cases)==0:
                        ...
                        hhs_with_atleastonecase+=1
                        if sum(num_cases)>=2:
                            hhs_with_atleasttwocases+=1
                            if sum(num_cases)>=3:
                                hhs_with_atleastthreecases+=1
                          
                self.stat['hh_with_zerocase']=hhs_with_zerocases/number_hhs
                self.stat['hh_with_atleastonecase']=hhs_with_atleastonecase/number_hhs
                self.stat['hh_with_atleasttwocase']=hhs_with_atleasttwocases/number_hhs
                self.stat['hh_with_atleastthreecase']=hhs_with_atleastthreecases/number_hhs
                self.stat.append()
                new_stats_df=pd.DataFrame({'t':[t], 
                       'overallprevalence':[len([i for i in list(pop.I.values()) if i.state.label == "I"])/len(list(pop.I.values()))],
                       'mean_age_infected':mean_age_infected,
                        'variance_age_infected':var_age_infected,
                           'mean_hhsize_infected':mean_hhsize_infected,
                             'hh_with_zerocase':[hhs_with_zerocases/number_hhs],
                                           'hh_with_atleastonecase':[hhs_with_atleastonecase/number_hhs],
                                           'hh_with_atleasttwocase':[hhs_with_atleasttwocases/number_hhs],
                                           'hh_with_atleastthreecase':[hhs_with_atleastthreecases/number_hhs],
                                           'com_transmission_percentage':com_transmisssion_perc
...
class SummaryStatsObserver3(Observer):
    """
    An observer for recording summary stats. new summary statistics hh with at least 1,2,3 cases are recorded.
    It collects data as the original data (unimputed) where only present people information is collected for the selected households
    """
    def __init__(self,h5file,observefrom,et,fname,rng,  t_per_year=364):
        self.rng=rng
        self.fname=fname
        ...
        super(SummaryStatsObserver3, self).__init__(h5file, 'stats', desc, 'Summary Stats') #??? 
    ...
    def update(self, t, disease, pop,cases, **kwargs):
        ...
        if t % self.t_per_year in range(0, self.t_per_year, self.t_per_year // 4):
            if t >= (self.observefrom):
                ...
                all_peoplein_sample_hhs=[i for i in list(pop.I.values()) if i.groups['household'] in sample_hh_ids]
                sampled_pop=[]    
                for hh in sample_hh_ids:
                    cur_hh_members=[i for i in list(pop.I.values()) if i.groups['household'] == hh]
                    sampled_pop.append(self.rng.sample(cur_hh_members,1)[0])
                
                left_peoplein_sample_hhs= [x for x in all_peoplein_sample_hhs if x not in sampled_pop]
                #display(len(sampled_pop))
                additional_samples=self.rng.sample(left_peoplein_sample_hhs,sample_size-sampled_hh) # sample more rows as needed
                #display(len(additional_samples))
                sampled_pop = sampled_pop+additional_samples
                no_hhs_sampled_pop=len(list(set([i.groups['household'] for i in sampled_pop])))
                #display(no_hhs_sampled_pop)
                #display(len(sampled_pop))
                #while True:
                ...
                #    no_hhs_sampled_pop=len(list(set([i.groups['household'] for i in sampled_pop])))
                    #display(no_hhs_sampled_pop)
                #    if no_hhs_sampled_pop in range(470, 490):
                #        break
                #display(no_hhs_sampled_pop)
                ...
                #other simple stats
                self.stat['t']=t
                self.stat['overallprevalence']=len([i for i in sampled_pop if i.state.label == "I"])/len(sampled_pop)
                ages_infected=np.array([i.age for i in sampled_pop if i.state.label == "I"])
                mean_age_infected=np.mean(ages_infected) if len(ages_infected)>0 else 0
                var_age_infected=np.var(ages_infected) if len(ages_infected)>0 else 0
                self.stat['mean_age_infected']=mean_age_infected
                ...
                #comm transmission in whole population
                inf_pop=[i for i in list(pop.I.values()) if i.state.label == "I"]
                com_transmisssion_perc=len([i for i in inf_pop if i.hh_source==0])/len(inf_pop) if len(inf_pop)> 0 else np.nan
                self.stat['com_transmission_percentage']=com_transmisssion_perc
                
                #self.hh_size_comm_dists_prevalence_df=pd.DataFrame()
                self.summarystats_df2=pd.DataFrame()
                #comm transmission in age groups
                ages_com_transmitted_inf_pop=np.array([i.age for i in inf_pop if i.hh_source==0])
                ages_com_transmitted_inf_pop_hist=np.histogram(ages_com_transmitted_inf_pop, bins=16, range=(0, 80))[0]
                ages_infected= np.array([i.age for i in inf_pop])
                ages_infected_hist=np.histogram(ages_infected, bins=16, range=(0, 80))[0]
                #division by zero error
                #age_dist_comm_transmission=ages_com_transmitted_inf_pop_hist/ages_infected_hist
                age_dist_comm_transmission=np.array([x/y if y else np.nan for x, y in zip(ages_com_transmitted_inf_pop_hist, ages_infected_hist)])
                ...
                #comm transmission in hh groups
                #hh_size_comm_trans=np.array([hh_size(pop, i) for i in list(pop.I.values()) if i.state.label == "I" and i.hh_source==0])
                hh_size_comm_trans=np.array([hh_size(pop, i) for i in inf_pop if i.hh_source==0])
                #display("ids of hh size comm trans inhhsize1")
                #display(np.array([[i.ID, i.groups['household']] for i in list(pop.I.values()) if i.state.label == "I" and i.hh_source==0 and hh_size(pop, i) == 1]))
                hh_size_comm_trans_hist=np.histogram(hh_size_comm_trans, bins=12, range=(1,13))[0]
                #hh_size_infected= np.array([hh_size(pop, i) for i in list(pop.I.values()) if i.state.label == "I"])
                ...
                for x in sample_hh_ids:
                    #display(x)
                    cur_hh_members=[i for i in sampled_pop if i.groups['household'] == x]
                    #cur_hh_members= sampled_pop.groups['household'][x]
                    #display(cur_hh_members)
                    num_cases=[1 if x.state.label=="I" else 0 for x in cur_hh_members]
                    #display(num_cases)
                ...
                self.stat.append()
                new_stats_df=pd.DataFrame({'t':[t], 
                       'overallprevalence':[len([i for i in sampled_pop if i.state.label == "I"])/len(sampled_pop)],
                       'mean_age_infected':mean_age_infected,
                        'variance_age_infected':var_age_infected,
                           'mean_hhsize_infected':mean_hhsize_infected,
                             'hh_with_zerocase':[hhs_with_zerocases/number_hhs],
