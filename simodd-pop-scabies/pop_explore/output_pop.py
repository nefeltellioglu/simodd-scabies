"""
Output functions for population data.
...
from population.pop_info import age_dist
from population.utils import load_probs, create_thumbnail
from pop_explore.data_processing_pop import *
from .plotting_pop import *


def output_html(p, ofile='summary.html'):
    try:
        from Cheetah.Template import Template

        figs = []
        for infile in sorted(glob(os.path.join(p['prefix'], '*.png'))):
            figs.append({
        ...
        t = Template(
            file=os.path.join(p['resource_prefix'], 'output.tmpl'),
            searchList=[
                {'p': p,
                 'images': figs,
                 'time': {
        ...
        with open(os.path.join(p['prefix'], ofile), 'w') as ofile:
            ofile.write(t)
    except ImportError:
        pass
...
def output_individual_logs(sim, ofile_prefix):
    ...
    dp.plot_age_dists(x, (marriage_1[0], c1f[0], death[0]),
                      ...
                      #'major life events',
                      None,
                      os.path.join(ofile_prefix, 'major_life_events.%s' % sim.p['ext']))
    ...
    dp.plot_age_dists(cx, (c1[0], c2[0], c3[0], c4[0], c5[0]),
                      ...
                      #'birth of children',
                      None,
                      os.path.join(ofile_prefix, 'birth_of_children.%s' % sim.p['ext']),
                      show_legend=False)
    ...
    dp.plot_age_dists(x, (marriage_1[0], marriage_2[0], marriage_3[0]),
                      ...
                      #'marriages',
                      None,
                      sim.p['prefix'] + 'marriagesi.%s' % sim.p['ext'])
...
def output_hh_life_cycle(sim, ofile):
    ...
    fig = plt.figure()
    ax = fig.add_subplot(111)
    hh_sizes = size_by_age(sim.P.households, conditions, sim.p['years'], sim.p['t_per_year'])
...
def output_hh_age_distribution(sim, ofile):
    ...
    fig = plt.figure()
    ax = fig.add_subplot(111)
    d, b = get_hh_age_dist(sim.P.households, conditions)  # , sim.p['t_per_year'])
    ax.bar(b[1:-1], d[1:])
...
def output_comp_by_hh_size(sim, ofile):
    ...
    fig = plt.figure()
    ax = fig.add_subplot(111)
    #print get_hh_comp_by_size(sim.P, list(cutoffs))
    plot_hh_comp_by_hh_size(
        ax, convert_counts_to_proportions(
            get_hh_comp_by_size(sim.P, list(cutoffs))),
        cutoffs, colours)
    fig.savefig(ofile)
...
def output_comp_by_hh_age(sim, ofile):
    ...
    fig = plt.figure()
    ax = fig.add_subplot(111)
    plot_hh_comp_by_hh_age(
        ax, convert_counts_to_proportions(
            get_hh_comp_by_age(
                sim.P, list(cutoffs),
                sim.p['years'], sim.p['t_per_year'], binsize=5)),
        cutoffs, colours, 5)
    fig.savefig(ofile)
...
def output_hh_size_distribution(sim, max_size, ofile):
    ...
    fig = plt.figure()
    ax = fig.add_subplot(111)
    if 'hh_composition_final' in sim.p:
        hh_dat = collapse_hh_size_probs(load_probs(os.path.join(
            sim.p['resource_prefix'], sim.p['hh_composition_final'])))
    else:
        hh_dat = collapse_hh_size_probs(load_probs(os.path.join(
            sim.p['resource_prefix'], sim.p['hh_composition'])))
...
def output_age_distribution(sim, ofile, num_samples=None):
    """
    ...
    """
    if not num_samples:
        num_samples = int(len(age_dist) * 0.2)
    n_means, n_stdev = get_mean_and_stdev(age_dist[-num_samples:])

    fig = plt.figure()
    ax = fig.add_subplot(111)
    if 'age_distribution_final' in sim.p:
        age_dat = np.array([x[0] for x in \
                            load_probs(os.path.join(
                                sim.p['resource_prefix'],
                                sim.p['age_distribution_final']))])
    else:
        age_dat = np.array([x[0] for x in \
                            load_probs(os.path.join(
                                sim.p['resource_prefix'], sim.p['age_distribution']))])
...
def output_household_transitions(ofile):
    fig = plot_hh_trans()
    fig.savefig(ofile)
